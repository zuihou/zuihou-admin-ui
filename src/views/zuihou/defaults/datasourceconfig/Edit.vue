  ...
  <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" :title="title" :visible.sync="isVisible" :width="width" top="50px">
    <el-form ref="form" :model="datasourceConfig" :rules="rules" label-position="right" label-width="130px">
      ...
      <el-form-item :label="$t('table.datasourceConfig.url')" prop="url" placeholder="连接信息">
        <el-tooltip class="item" content="确保租户服务和数据库ip网络互通;数据库命名规范参考：zuihou_base_{TENANT}, zuihou_base要和zuihou.database.tenantDatabasePrefix保持一致" effect="dark" placement="right-start">
      ...
      <aside style="word-wrap: break-word">
        ...
        链接中的数据库命名规范建议：zuihou_base_{TENANT}, zuihou_base要和zuihou.database.tenantDatabasePrefix保持一致，否则删除租户时，无法删除库
      </aside>
      <aside style="word-wrap: break-word">
        eg: 1111 租户需要配置2个链接<br>
        名称：1111-基础服务<br>
        链接：jdbc:mysql://127.0.0.1:3306/zuihou_base_1111?serverTimezone=CTT&characterEncoding=utf8&useUnicode=true&useSSL=false&autoReconnect=true&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true<br>

        名称：1111-扩展服务<br>
        链接：jdbc:mysql://127.0.0.1:3306/zuihou_extend_1111?serverTimezone=CTT&characterEncoding=utf8&useUnicode=true&useSSL=false&autoReconnect=true&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true<br>

        其中:<br>
        1111-基础服务：用于 认证服务数据源、权限服务数据源、文件服务数据源、消息服务数据源， 所以这些服务的配置文件中需要配置 zuihou.database.tenantDatabasePrefix = zuihou_base<br>
        1111-扩展服务：用于 网关服务数据源， 所以这些服务的配置文件中需要配置 zuihou.database.tenantDatabasePrefix = zuihou_extend
      </aside>
    </el-form>
  ...
  methods: {
    initDatasourceConfig () {
      return {
        ...
        url: 'jdbc:mysql://127.0.0.1:3306/zuihou_base_1111?serverTimezone=CTT&characterEncoding=utf8&useUnicode=true&useSSL=false&autoReconnect=true&zeroDateTimeBehavior=convertToNull&allowMultiQueries=true'
      }
    },
    initWidth () {
